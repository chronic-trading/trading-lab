...
const tierConfig = {
  basic: {
    ...
    border:         'border-[var(--border)] hover:border-emerald-500/40',
    selectedBorder: 'border-emerald-500/60',
    badge:          'text-emerald-300 bg-emerald-500/12 border border-emerald-500/25',
    addBtnIdle:     'border-[var(--border)] text-[var(--text-dim)] hover:border-emerald-500/50 hover:text-emerald-400 hover:bg-emerald-500/10',
  ...
  intermediate: {
    ...
    border:         'border-[var(--border)] hover:border-blue-500/40',
    selectedBorder: 'border-blue-500/60',
    badge:          'text-blue-300 bg-blue-500/12 border border-blue-500/25',
    addBtnIdle:     'border-[var(--border)] text-[var(--text-dim)] hover:border-blue-500/50 hover:text-blue-400 hover:bg-blue-500/10',
  ...
  advanced: {
    ...
    border:         'border-[var(--border)] hover:border-purple-500/40',
    selectedBorder: 'border-purple-500/60',
    badge:          'text-purple-300 bg-purple-500/12 border border-purple-500/25',
    addBtnIdle:     'border-[var(--border)] text-[var(--text-dim)] hover:border-purple-500/50 hover:text-purple-400 hover:bg-purple-500/10',
...
export function ConceptCard({ concept, selected, onToggle }: Props) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-[var(--bg-elev)] -z-10" />
      ...
      <div className="relative pl-5 pr-5 pt-4 pb-4">
        ...
        {/* Name */}
        <h3 className="text-[15px] font-bold text-[var(--text)] leading-snug mb-2 pr-1 tracking-tight">{concept.name}</h3>
      ...
      <AnimatePresence>
        {expanded && (
          ...
          >
            <div className="relative px-5 pb-4 pt-3.5 border-t border-[var(--border)]">
              <div className="absolute inset-0 bg-[var(--surface)]" />
              <div className="relative">
                <p className="text-[13px] text-[var(--text)] leading-relaxed"><GlossaryText text={concept.howToUse} /></p>
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {concept.tags.map(t => (
                    <span key={t} className="text-[10px] font-medium px-2 py-0.5 rounded-full bg-[var(--surface-2)] text-[var(--text-dim)] border border-[var(--border)]">#{t}</span>
                  ))}
                </div>
              </div>
            </div>
          ...
          >
            ...
            >
              ...
              <textarea
                ...
                rows={3}
                className="w-full bg-[var(--surface)] border border-[var(--border)] rounded-xl px-3 py-2.5 text-[12px] text-[var(--text)] placeholder-[var(--text-faint)] focus:outline-none focus:border-amber-500/40 transition-colors resize-none"
                onClick={e => e.stopPropagation()}
              />
              {note && (
                <p className="text-[10px] text-[var(--text-faint)] mt-1.5">Saved automatically</p>
              )}
            </div>
          </motion.div>
        )}
