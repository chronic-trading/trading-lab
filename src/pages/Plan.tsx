...
const BiasBtn = ({ label, value, current, onChange, icon: Icon, color }: {
  ...
  <button
    ...
    className={`flex-1 flex flex-col items-center gap-1.5 py-3 rounded-2xl border text-[11px] font-semibold transition-all
      ${current === value ? color : 'border-[var(--border)] text-[var(--text-faint)] hover:border-[var(--border)] hover:text-[var(--text-dim)]'}`}
  >
    <Icon size={16} />
    {label}
  </button>
  ...
  return (
    <button
      ...
      className={`flex-1 flex flex-col items-center gap-0.5 py-2 rounded-xl border text-[10px] font-bold transition-all
        ${current === phase ? 'border-amber-500/50 bg-amber-500/10 text-amber-300' : 'border-[var(--border)] text-[var(--text-faint)] hover:border-[var(--border)]'}`}
    >
  ...
  return (
    <button
      ...
      className={`flex-1 py-2 rounded-xl border text-[11px] font-semibold transition-all
        ${on ? 'border-emerald-500/45 bg-emerald-500/10 text-emerald-300' : 'border-[var(--border)] text-[var(--text-faint)] hover:border-[var(--border)]'}`}
    >
      {label}
    </button>
  )
...
function RiskCalc({ defaultInstrument }: { defaultInstrument: Instrument }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Spec info */}
      <div className="bg-[var(--surface)] border border-[var(--border)] rounded-xl px-3 py-2 flex items-center gap-4">
        <span className="text-[11px] text-[var(--text-dim)]">{spec.name}</span>
        <span className="text-[11px] text-[var(--text-faint)]">·</span>
        <span className="text-[11px] text-[var(--text-dim)]">${spec.pointValue}/pt</span>
        <span className="text-[11px] text-[var(--text-faint)]">·</span>
        <span className="text-[11px] text-[var(--text-dim)]">${spec.tickValue}/tick</span>
      </div>
      ...
      <div className="space-y-3">
        {[
          ...
          <div key={label} className="space-y-1.5">
            <label className="text-[11px] font-semibold text-[var(--text-dim)]">{label}</label>
            <div className="relative">
              <input
                ...
                className="w-full bg-[var(--surface)] border border-[var(--border)] rounded-xl px-3 py-2.5 text-[13px] text-[var(--text)] placeholder-[var(--text-faint)] focus:outline-none focus:border-[var(--border-strong)] transition-colors pr-12"
                style={{ fontFamily: "'JetBrains Mono', monospace" }}
              />
              <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] text-[var(--text-faint)] font-bold">{prefix}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="tl-card p-4 space-y-3">
        ...
        <div className="flex justify-between items-center">
          <span className="text-[12px] text-[var(--text-dim)]">Contracts</span>
          <span className={`text-[22px] font-black ${contracts > 0 ? 'text-amber-300' : 'text-[var(--text-faint)]'}`} style={{ fontFamily: "'JetBrains Mono', monospace" }}>
            {contracts}
          </span>
        </div>
        <div className="flex justify-between items-center">
          <span className="text-[11px] text-[var(--text-dim)]">Actual Risk ({contracts} contracts)</span>
        ...
        {stopPts && contracts === 0 && parseFloat(stopPts) > 0 && (
          <p className="text-[11px] text-[var(--text-faint)] text-center">Your risk % is too low or stop too wide for 1 full contract</p>
        )}
      </div>
    </div>
  )
...
export function Plan() {
  ...
  const biasColor = (b: Bias) => b === 'bullish' ? 'text-emerald-400' : b === 'bearish' ? 'text-red-400' : 'text-[var(--text-dim)]'
    ...
    <div className="flex h-full overflow-hidden flex-col md:flex-row">
            ...
            {([
              ...
              <button
                ...
                className={`flex items-center gap-2 px-4 py-3 text-[12px] font-semibold border-b-2 transition-all
                  ${view === id ? 'border-amber-400 text-[var(--text)]' : 'border-transparent text-[var(--text-dim)] hover:text-[var(--text-dim)]'}`}
              >
                <Icon size={13} />
                {label}
              </button>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {view === 'plan' && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  <label className="text-[11px] font-bold text-[var(--text-dim)] uppercase tracking-wider">Daily Phase</label>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <div className="flex gap-1.5">
                    ...
                    <BiasBtn label="Neutral" value="neutral" current={weeklyBias} onChange={setWeeklyBias} icon={Minus} color="border-[var(--border-strong)] bg-[var(--surface-hover)] text-[var(--text-dim)]" />
                  </div>
                </div>
                <div className="space-y-2">
                  <label className="text-[11px] font-bold text-[var(--text-dim)] uppercase tracking-wider">Daily Bias</label>
                  <div className="flex gap-1.5">
                    ...
                    <BiasBtn label="Neutral" value="neutral" current={dailyBias} onChange={setDailyBias} icon={Minus} color="border-[var(--border-strong)] bg-[var(--surface-hover)] text-[var(--text-dim)]" />
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-[var(--text-dim)] uppercase tracking-wider">Bias Reasoning</label>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <label className="text-[11px] font-bold text-[var(--text-dim)] uppercase tracking-wider">Key Levels to Mark</label>
                  <button onClick={addLevel} className="flex items-center gap-1 text-[10px] text-[var(--text-faint)] hover:text-amber-400 transition-colors">
                  ...
                  {keyLevels.map((lvl, i) => (
                    <div key={i} className="flex gap-2">
                      <input value={lvl} onChange={e => updateLevel(i, e.target.value)}
                        ...
                        className="flex-1 bg-[var(--surface)] border border-[var(--border)] rounded-xl px-3 py-2 text-[12px] text-[var(--text)] placeholder-[var(--text-faint)] focus:outline-none focus:border-[var(--border-strong)] transition-colors" />
                      {keyLevels.length > 1 && (
                        <button onClick={() => removeLevel(i)} className="w-8 h-8 flex items-center justify-center rounded-xl text-[var(--text-faint)] hover:text-red-400 hover:bg-red-400/10 transition-all flex-shrink-0">
                          <Trash2 size={12} />
                        </button>
                      )}
                    </div>
              ...
              {/* DOL */}
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-[var(--text-dim)] uppercase tracking-wider flex items-center gap-2">
                ...
                <input value={dol} onChange={e => setDol(e.target.value)}
                  ...
                  className="w-full bg-[var(--surface)] border border-amber-500/25 rounded-xl px-3.5 py-2.5 text-[13px] text-[var(--text)] placeholder-[var(--text-faint)] focus:outline-none focus:border-amber-500/50 transition-colors" />
              </div>
              ...
              {/* Max trades */}
              <div className="space-y-2">
                <label className="text-[11px] font-bold text-[var(--text-dim)] uppercase tracking-wider">Max Trades Today</label>
                <div className="flex gap-1.5">
                  {[1, 2, 3, 4].map(n => (
                    <button key={n} onClick={() => setMaxTrades(n)}
                      className={`flex-1 py-2 rounded-xl border text-[12px] font-bold transition-all
                        ${maxTrades === n ? 'bg-[var(--surface-hover)] border-[var(--border-strong)] text-[var(--text)]' : 'border-[var(--border)] text-[var(--text-faint)] hover:border-[var(--border-strong)]'}`}
                      style={{ fontFamily: "'JetBrains Mono', monospace" }}
                    >{n}</button>
                  ))}
                </div>
              ...
              {/* Avoid */}
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-[var(--text-dim)] uppercase tracking-wider">Today I Will NOT</label>
                  ...
                  <div className="space-y-1.5">
                    {activeRules.map(r => (
                      <div key={r.id} className="flex items-start gap-2">
                        <div className="w-1 h-1 rounded-full bg-[var(--border-strong)] mt-2 flex-shrink-0" />
                        <p className="text-[12px] text-[var(--text-dim)] leading-relaxed">{r.text}</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {/* Notes */}
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-[var(--text-dim)] uppercase tracking-wider">Additional Notes</label>
          ...
          {view === 'calc' && (
            <div className="max-w-md mx-auto">
              <h2 className="text-[16px] font-bold text-[var(--text)] mb-5">Position Size Calculator</h2>
                  ...
                  <div key={p.id} className="tl-card p-4 space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <button onClick={() => { setPlans(pl => { const u = pl.filter(x => x.id !== p.id); savePlans(u); return u }) }}
                        className="w-6 h-6 flex items-center justify-center rounded-lg text-[var(--text-faint)] hover:text-red-400 hover:bg-red-400/10 transition-all">
                        <Trash2 size={11} />
                      </button>
                    </div>
                    {p.drawOnLiquidity && (
                    ...
                    {p.reasoning && <p className="text-[12px] text-[var(--text-dim)] leading-relaxed">{p.reasoning}</p>}
                    {p.keyLevels.length > 0 && (
                      <div className="flex flex-wrap gap-1.5">
                        {p.keyLevels.map((l, i) => <span key={i} className="text-[10px] text-[var(--text-dim)] bg-[var(--surface-2)] border border-[var(--border)] rounded-lg px-2 py-0.5">{l}</span>)}
                      </div>
                    )}
                  </div>
                ))
      ...
      <div className="hidden md:block w-[280px] flex-shrink-0 bg-[var(--bg-elev)] p-5 space-y-4 overflow-y-auto border-l border-[var(--border)]">
        ...
        <div className="tl-card p-4 space-y-3">
          ...
          <div className="flex items-center justify-between">
            <span className="text-[11px] text-[var(--text-dim)]">Daily Bias</span>
            <span className={`text-[12px] font-bold capitalize ${biasColor(dailyBias)}`}>{dailyBias}</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-[11px] text-[var(--text-dim)]">Weekly Bias</span>
            <span className={`text-[12px] font-bold capitalize ${biasColor(weeklyBias)}`}>{weeklyBias}</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-[11px] text-[var(--text-dim)]">Daily Phase</span>
            <span className="text-[12px] font-bold text-amber-400">{dailyPhase}</span>
          </div>
          <div className="flex items-center justify-between">
            <span className="text-[11px] text-[var(--text-dim)]">Max Trades</span>
            <span className="text-[12px] font-bold text-[var(--text)]" style={{ fontFamily: "'JetBrains Mono', monospace" }}>{maxTrades}</span>
          </div>
          {dol && (
        ...
        {killZones.length > 0 && (
          <div>
            <p className="text-[10px] font-bold text-[var(--text-faint)] uppercase tracking-wider mb-2">Trading Today</p>
...
function KeyLevelsWidget({ currentInstrument }: { currentInstrument: Instrument }) {
  ...
  const typeConfig = {
    ...
    neutral:    { label: 'N', dot: 'bg-[var(--border-strong)]',   active: 'bg-[var(--surface-hover)] border-[var(--border-strong)] text-[var(--text)]'          },
  }
    ...
    <div className="border-t border-[var(--border)] pt-4 space-y-3">
      ...
      <div className="flex gap-1.5">
        ...
        <input value={label} onChange={e => setLabel(e.target.value)} onKeyDown={e => e.key === 'Enter' && submit()}
          placeholder="Label" className="flex-1 min-w-0 bg-[var(--surface)] border border-[var(--border)] rounded-xl px-2.5 py-1.5 text-[11px] text-[var(--text)] placeholder-[var(--text-faint)] focus:outline-none focus:border-[var(--border-strong)] transition-all" />
        <input value={price} onChange={e => setPrice(e.target.value)} onKeyDown={e => e.key === 'Enter' && submit()}
          placeholder="Price" className="w-20 flex-shrink-0 bg-[var(--surface)] border border-[var(--border)] rounded-xl px-2.5 py-1.5 text-[11px] text-[var(--text)] placeholder-[var(--text-faint)] focus:outline-none focus:border-[var(--border-strong)] transition-all font-mono" />
      </div>
      <div className="flex gap-1.5 items-center">
        {(['resistance', 'support', 'neutral'] as const).map(t => (
          <button key={t} onClick={() => setType(t)}
            className={`flex-1 py-1 rounded-xl border text-[10px] font-bold transition-all ${type === t ? typeConfig[t].active : 'border-[var(--border)] text-[var(--text-faint)] hover:border-[var(--border)]'}`}>
            {typeConfig[t].label}
          </button>
        ))}
      ...
      <div className="space-y-1">
        {current.length === 0
          ? <p className="text-[10px] text-[var(--text-faint)] text-center py-1">No levels for {inst}</p>
          : current.map(lv => (
            <div key={lv.id} className="flex items-center gap-2 bg-[var(--surface)] border border-[var(--border)] rounded-xl px-2.5 py-1.5">
              <div className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${typeConfig[lv.type].dot}`} />
              <span className="text-[11px] font-semibold text-[var(--text-dim)] flex-1 truncate">{lv.label}</span>
              <span className="text-[11px] text-[var(--text-dim)]" style={{ fontFamily: "'JetBrains Mono', monospace" }}>{lv.price}</span>
              <button onClick={() => remove(inst, lv.id)} className="text-[var(--text-faint)] hover:text-red-400 transition-colors flex-shrink-0">
                <X size={9} />
              </button>
            </div>
          ))
        ...
        {current.length > 0 && (
          <button onClick={() => clear(inst)} className="text-[10px] text-[var(--text-faint)] hover:text-red-400 transition-colors w-full text-center pt-0.5">
            clear {inst}
          </button>
        )}
      </div>
